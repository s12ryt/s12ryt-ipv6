...
grep -Fq 'contents: read' "$WORKFLOW" || fail "release workflow does not default to read-only contents permission"
grep -Fq 'contents: write' "$WORKFLOW" || fail "publishing job lacks contents write permission"
grep -Fq 'needs: verify' "$WORKFLOW" || fail "publishing job does not depend on release verification"
...
grep -Fq 'npm audit --audit-level=high' "$WORKFLOW" || fail "release workflow does not audit frontend dependencies"
...
grep -Fq 'go test -race -shuffle=on ./...' "$WORKFLOW" || fail "release workflow does not run shuffled Go race tests"
grep -Fq 'run: go test .' "$WORKFLOW" || fail "release workflow does not test the web embed module"
grep -Fq 'govulncheck@v1.1.4 ./...' "$WORKFLOW" || fail "release workflow does not scan Go vulnerabilities"
...
grep -Fq 'goreleaser/goreleaser-action@e435ccd777264be153ace6237001ef4d979d3a7a # v6.4.0' "$WORKFLOW" || fail "GoReleaser action is not pinned to v6.4.0"
grep -Fq 'version: v2.18.1' "$WORKFLOW" || fail "GoReleaser binary is not pinned to v2.18.1"
...
grep -Fq 'echo "create-tag=$create_tag" >>"$GITHUB_OUTPUT"' "$WORKFLOW" || fail "manual workflow does not publish whether a tag must be created"
...
grep -Fq "needs.verify.outputs.create-release-tag == 'true'" "$WORKFLOW" || fail "manual tag creation is not guarded by the verification output"
grep -Fq 'GORELEASER_CURRENT_TAG: ${{ needs.verify.outputs.release-tag }}' "$WORKFLOW" || fail "GoReleaser is not pinned to the verified tag"
if grep -Fq "if: github.event_name == 'push'" "$WORKFLOW"; then
    fail "manual workflow still skips GitHub Release publishing"
fi
grep -Fq 'name: Upload verified web assets' "$WORKFLOW" || fail "verified frontend artifact is not uploaded"
grep -Fq 'name: Restore verified web assets' "$WORKFLOW" || fail "publishing job does not restore the verified frontend artifact"
