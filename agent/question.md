...
### 44. v1.0.9 更新後節點啟動失敗（2026-09-13）

使用者回報：「更新版本後節點無法開啟」。初次詢問得到「啟動後仍停止」與 `node operation failed`；之後使用者更正為「重啟就正常了，原因大概是不能在運行時熱更新系統」。最終可觀察症狀與驗收契約以此更正為準：

1. 更新版本為目前 GitHub latest `v1.0.9`。
2. 更新後的第一次服務進程中節點操作異常；手動重啟服務後恢復正常。
3. 官方一鍵與離線安裝器必須經程式碼及腳本測試確認會停止舊服務、替換 binary／unit、執行 `daemon-reload` 並啟動新進程；已被證據否定的「舊進程熱更新」不得當成根因。
4. 首次新進程啟動時，資源對帳與期望為 running 的節點若遇短暫錯誤，必須在同一進程內有界重試，不應要求使用者再手動重啟一次。
5. 手動啟動 stopped 節點也必須套用相同的有界重試；重試等待必須可被 context 取消，永久錯誤不得形成無限重試或 systemd 重啟迴圈。
6. 修復必須分別以 RED 重現資源對帳、開機恢復與手動啟動的一次性失敗，再做最小 GREEN；沒有 VPS journal 時，不得聲稱已確認底層第一次錯誤的種類。

### 45. Watchdog 跨程序重啟保護（2026-09-13）

使用者回報：「這個看門狗程式似乎會一直重啟程式不管成功與否」。驗收契約：

1. 同一資料面故障 episode 最多自動要求一次服務重啟；重啟命令成功只代表 systemd 接受命令，不代表資料面已恢復。
2. 重啟保護必須保存於資料目錄並跨程序生效，新程序不得因記憶體冷卻時間歸零而再次重啟。
3. 新程序仍須持續探測造成重啟的同一 target；只有該 target 實際探測成功，才能清除保護並允許未來的新故障 episode 再次自癒。
4. 被保護 target 已移除時清除過期保護；重啟命令本身失敗時清除保護並沿用有界 cooldown。
5. 保護狀態讀取或保存失敗時採 fail closed，不得在無法證明已有保護的情況下重啟；錯誤必須留下不含代理帳密的 system event。
6. 狀態檔使用嚴格版本化 JSON、原子替換與 Linux `0600` 權限，只保存 node、listener address、protocol 與重啟時間。

### 46. Watchdog 兩層綜合健康判定（2026-09-13）

使用者指出：「看門狗還有更多東西沒有完善，例如好像只要有一個測試對象失效就一律視為失敗」。經詢問後，使用者選定「網站目的與代理節點兩層都完善」，且兩層皆採「任一成功即健康」。驗收契約：

1. 單一代理目標內的 12 個原生 IPv6 目的，以及啟用時額外的 DNS64／NAT64 目的，維持全部實際執行；任一目的成功即代表該代理目標可用，只有全部目的失敗才回傳該目標失敗。
2. 所有 running 代理 listener 共同代表整體資料面；任一 listener 探測成功即清除本次全域失敗 episode，不得因另一個 listener 的局部故障重啟整個服務。
3. watchdog 每輪只探測一個 listener，失敗後必須優先探測尚未達同等失敗次數的其他 listener；所有目前 listener 都各自連續達到門檻後，才可要求全域重啟。
4. 跨程序 restart guard 代表全域故障 episode，只保留觸發重啟的 listener 作診斷資訊；新程序不得永遠黏住該 listener，任一現存 listener 成功都必須清除 guard。
5. listener 新增或移除時，失敗狀態須依目前 target 集合重新收斂；不得因已移除的局部故障觸發重啟，也不得讓新 target 未經探測就被視為失敗。
6. 必須以 RED 分別證明「一壞一好不重啟」、「全部 target 各達門檻才重啟」及「不同 target 的成功解除跨程序 guard」，再做最小 GREEN。

## 2026-09-22 第三十二輪：IP 池輪換「位址回收重用」修復（TDD 驗收標準）

### 使用者提問與決定
- 問題：`Store.RefreshPool` 用 `GenerateAddresses` 從前綴最低位址開始掃，drain 完成後舊位址會從 store 移除，因此下一次 refresh 又會取回剛釋放的低位址（實務上在兩組之間 A/B 交替重用）。
- 使用者問「3-2的那個能修嗎」，並經選項確認：**(1) 循序游標往前推進**、**(2) 游標要持久化（跨服務重啟保留）**。

### 驗收標準（TDD）
1. 產生自動位址時，改由「該前綴的下一個候選位置」開始往高位址方向掃描並跳過 occupied；到前綴尾端繞回前綴開頭繼續；整圈都找不滿 count 才回錯誤。
2. 每次成功產生後，該前綴的游標前進到「最後取得的位址 + 1」；若已到前綴尾端則繞回前綴開頭。
3. 既有行為不變：游標未設定（例如新建池第一次產生、或由舊狀態檔載入）時仍從前綴最低可用位址開始。
4. `RefreshPool` 在舊批次 drain 完成（舊位址已從 store 移除）之後再次刷新時，**不得**再取得先前用過的位址。
5. 游標需持久化於資源狀態（`resources.yaml`）；舊狀態檔（無游標欄位）必須能載入，視為未設定，不得破壞 schema 相容。
6. 前綴耗盡（游標繞完一圈仍不足 count）時回明確錯誤，不得無限迴圈。
7. pinned 位址不受影響（永不輪換、不參與游標計算）；`SourcePool` 的 round-robin 選址與 drain 完成協定皆不改動。